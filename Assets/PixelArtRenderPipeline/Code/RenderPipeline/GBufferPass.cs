    ...
    public class GBufferPass
    {
        public static void DrawRenderers(ScriptableRenderContext context, Camera camera, MultipleRenderTarget gBuffer)
        {
            ...
            foreach (var renderer in PixelArtRenderer.PixelArtRenderers)
            {
                ...
                cmd.GetTemporaryRT(Shader.PropertyToID("_Albedo_proxy"), width,height,0,FilterMode.Point, gBuffer.AlbedoRt.descriptor.graphicsFormat);
                cmd.GetTemporaryRT(Shader.PropertyToID("_Normal_proxy"), width,height,0,FilterMode.Point, gBuffer.NormalRt.descriptor.graphicsFormat);
                cmd.GetTemporaryRT(Shader.PropertyToID("_Depth_proxy"), width,height, gBuffer.DepthRt.descriptor.depthBufferBits,FilterMode.Point, gBuffer.DepthRt.descriptor.graphicsFormat);
